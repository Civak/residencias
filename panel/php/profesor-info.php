...
class Profesor{
    ...
    public function misGrupos() {
 	   ...
 	   $cat = ''; 	
 	   $encontrado = false; 	
    		$sql = "SELECT profesores.nombre, profesores.foto FROM profesores WHERE profesores.rfc = '".$_SESSION['profesor']."'";
			$result = $this->conn->query($sql);

			if ($result->num_rows > 0) {
			    while($row = $result->fetch_assoc()) {
			        $cat = "<img src='img/".$row['foto']."' alt='Foto de ".$row['nombre']."'><span>".$row['nombre']."</span>";
			    }
			   $encontrado = true;
			} 
    	...
    	public function tareasPendientes(){
			...
			if ($result->num_rows > 0) {
			   while($row = $result->fetch_assoc()) {
			        $cat .= "<div data-component='modal' data-target='#tareas-info' id='act-".$row['id']."' class='tareas box-tareas' title='Grupo: ".$row['grupo']."'><i class='fa fa-clipboard'></i> ".substr($row['titulo'], 0, 20)."...<br>";
